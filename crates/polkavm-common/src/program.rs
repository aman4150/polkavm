...
impl ProgramBlob {
    ...
    pub fn estimate_interpreter_memory_usage<I: InstructionSet>(
        ...
        let instruction_count = program_info.instruction_count;
        let basic_block_count = program_info.basic_block_count;
        let max_block_size = program_info.max_block_size;
        let page_size = program_info.page_size;
        ...
        let cache_bytes_upper_bound = interpreter_calculate_cache_size(cache_entry_count_upper_bound);

        let purgeable_ram_consumption = if let Some(max_cache_size_bytes) = max_cache_size_bytes {
            if max_block_size == 0 {
