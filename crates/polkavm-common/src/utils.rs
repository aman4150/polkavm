...
impl ArcBytes {
    ...
    pub(crate) fn parent_address_range(&self) -> Range<usize> {
        match &self.lifetime {
            LifetimeObject::None => 0..0,
            LifetimeObject::Arc { obj } => {
                let full_slice = obj.as_ref();
                full_slice.as_ptr() as usize..(full_slice.as_ptr() as usize + full_slice.len())
            }
            LifetimeObject::Other { obj } => {
                let slice = obj.as_ref().as_ref();
