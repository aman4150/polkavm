...
use NonZeroU32 as FlatMapContainer;
...
pub(crate) struct InterpretedInstance {
    ...
    cycle_counter: u64,
    gas: i64,
    compiled_offset_for_block: FlatMap<FlatMapContainer, true>,
...
impl InterpretedInstance {
    pub fn new_from_module(module: Module, force_step_tracing: bool) -> Self {
        ...
        polkavm_common::static_assert!(core::mem::size_of::<FlatMapContainer>() == cast(INTERPRETER_FLATMAP_ENTRY_SIZE).to_usize());
    ...
    pub fn set_interpreter_cache_size_limit(&mut self, cache_info: Option<SetCacheSizeLimitArgs>) -> Result<(), Error> {
        let Some(cache_info) = cache_info else {
            self.max_compiled_handlers = None;
            return Ok(());
        };

        let max_block_size = cache_info.max_block_size();
        let max_cache_size_bytes = cache_info.max_cache_size_bytes();
        ...
        if compiled_handlers_hard_limit < minimum_compiled_handlers {
            ...
            return Err(Error::from("Interpreter cache size is too small to guarantee a tight upper bound"));
        }
    ...
    fn compile_block<const DEBUG: bool>(&mut self, program_counter: ProgramCounter) -> Option<Target> {
        ...
        if let Some(max_compiled_handlers) = self.max_compiled_handlers {
            let num_handlers = self.compiled_handlers.len() - cast(origin).to_usize();
            if num_handlers + self.min_compiled_handlers > max_compiled_handlers {
                log::trace!(
                    "Compiler handlers cache is too small, this will lead to infinite loop: {} + {} > {}; not resetting cache",
                    num_handlers,
