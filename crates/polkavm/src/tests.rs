...
use crate::{
    BackendKind, CallError, Caller, Config, Engine, GasMeteringKind, InterpreterCacheInfo, InterruptKind, Linker, MemoryAccessError,
    Module, ModuleConfig, ProgramBlob, ProgramCounter, Reg, Segfault,
};
...
fn bounded_interpreter_cache(config: Config) {
    ...
    instance.set_interpreter_cache_size(Some(InterpreterCacheInfo {
        max_block_size: 4,
        max_cache_size_bytes: 24 * 20,
    }));
